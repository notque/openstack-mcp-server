...
build-all: build/openstack-mcp-server

build/openstack-mcp-server: FORCE
	env $(GO_BUILDENV) go build $(GO_BUILDFLAGS) -ldflags '-s -w $(GO_LDFLAGS)' -o build/openstack-mcp-server ./cmd/openstack-mcp-server
...
install: FORCE build/openstack-mcp-server
	install -d -m 0755 "$(DESTDIR)$(PREFIX)/bin"
	install -m 0755 build/openstack-mcp-server "$(DESTDIR)$(PREFIX)/bin/openstack-mcp-server"
...
goimports: FORCE install-goimports
	@printf "\e[1;36m>> goimports -w -local https://github.com/notque/openstack-mcp-server\e[0m\n"
	@goimports -w -local github.com/notque/openstack-mcp-server $(patsubst $(shell awk '$$1 == "module" {print $$2}' go.mod)%,.%/*.go,$(shell go list ./...))
...
help: FORCE
	...
	@printf "  \e[36mbuild/openstack-mcp-server\e[0m  Build openstack-mcp-server.\n"
